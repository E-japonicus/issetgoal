...
* Rename the newmodule/ folder to the name of your module (eg "widget").
  ...
  newmodule /フォルダの名前をあなたのモジュールの名前に変更します（例えば、 "widget"）。
...
* Edit all the files in this directory and its subdirectories and change
  ...
  find . -type f -exec sed -i 's/newmodule/issetgoal/g' {} \;
  find . -type f -exec sed -i 's/NEWMODULE/ISSETGOAL/g' {} \;
...
  あなたのモジュール名に文字列 "newmodule"のすべてのインスタンス
...
* Rename the file lang/en/newmodule.php to lang/en/widget.php
  ...
  ファイルlang / en / newmodule.phpの名前をlang / en / widget.phpに変更します
...
* Rename all files in backup/moodle2/ folder by replacing "newmodule" with
  ...
  backup / moodle2 /フォルダ内のすべてのファイルの名前を、 "newmodule"を
  あなたのモジュールの名前
  Linuxでは、以下の関数を呼び出すことでこの手順と前の手順を実行できます。
  $ find . -depth -name '*newmodule*' -execdir bash -c 'mv -i "$1" "${1//newmodule/widget}"' bash {} \;

  find . -depth -name '*newmodule*' -execdir bash -c 'mv -i "$1" "${1//newmodule/issetgoal}"' bash {} \;
